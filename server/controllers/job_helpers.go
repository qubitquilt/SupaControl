...
func (r *SupabaseInstanceReconciler) createProvisioningJob(ctx context.Context, instance *supacontrolv1alpha1.SupabaseInstance) (*batchv1.Job, error) {
	...
	err := r.Get(ctx, client.ObjectKey{Namespace: namespace, Name: jobName}, existingJob)
	if err == nil {
...
echo "[1/4] Secrets created successfully"

# Step 2: Add Helm repository
echo "[2/4] Adding Helm repository: $CHART_REPO"
helm repo add supabase-community "$CHART_REPO" || true
helm repo update

# Step 3: Install Helm chart
echo "[3/4] Installing Helm chart: $CHART_NAME (version: $CHART_VERSION)"
...
echo "[3/4] Helm chart installed successfully"
...
`},
	...
	logger.Info("Created provisioning Job", "jobName", jobName, "namespace", namespace)
	return job, nil
}
...
echo "[1/2] Uninstalling Helm release: $RELEASE_NAME"
if helm list -n "$NAMESPACE" | grep -q "$RELEASE_NAME"; then
  helm uninstall "$RELEASE_NAME" --namespace "$NAMESPACE" --wait --timeout 5m || true
  echo "[1/2] Helm release uninstalled"
else
  echo "[1/2] Helm release not found, skipping"
fi

# Step 2: Report completion
echo "[2/2] Cleanup complete!"
echo "========================================"
echo "Instance '$INSTANCE_NAME' resources within namespace have been deleted"
echo "========================================"
`},
							Env: []corev1.EnvVar{
								{
	...
	logger.Info("Created cleanup Job", "jobName", jobName, "namespace", namespace)
	return job, nil
}

// getJobStatus retrieves the status of a Job
func (r *SupabaseInstanceReconciler) getJobStatus(ctx context.Context, namespace, jobName string) (*batchv1.Job, error) {
	job := &batchv1.Job{}
